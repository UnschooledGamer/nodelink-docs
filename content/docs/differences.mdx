...
Stream audio directly as binary data or get a redirect to the source URL. Enable in config with `enableTrackStreamEndpoint: true`.

<Tabs items={['NodeLink', 'Lavalink']}>
  <Tab value="NodeLink">
```bash
# Works with ANY source (YouTube, Spotify, Deezer, etc.)
curl "http://your-server.com/v4/trackstream?encodedTrack=QAAAjQI..." \
     -H "Authorization: yourPassword" \
     --output audio.mp3
```
  </Tab>
...
<Tabs items={['Direct Playback', 'Mirror Sources', 'Search Prefixes']}>
  <Tab value="Direct Playback">
    ...
    | NicoVideo | ❌ Not available | ✅ Built-in |
    | Pandora | ❌ Not available | ✅ Built-in |
    | JioSaavn | ❌ Not available | ✅ Built-in |
    | Local Files | ✅ Built-in | ✅ Built-in |
    | HTTP URLs | ✅ Built-in | ✅ Built-in |

    *Requires [YouTube Plugin](https://github.com/lavalink-devs/youtube-source) (community-maintained)  
    **Requires [LavaSrc Plugin](https://github.com/topi314/LavaSrc) (community-maintained)  
    ***Requires Deezer ARL token (free to obtain)
  </Tab>
  <Tab value="Mirror Sources">
    ...
    | Tidal | ❌ Not available | ✅ Built-in | Requires web token (free) |
