...
```
  ...
  <ApiSection title="Example Request">
```bash
curl -X GET "http://your-server.com/v4/trackstream?encodedTrack=QAAAjQIAJVJpY2sgQXN0bGV5...&itag=251" \
     -H "Authorization: yourPassword" \
     --output audio.opus
```
  </ApiSection>
...
Enable this feature carefully as it allows direct audio downloads. Consider rate limiting and authentication.
  ...
  <ApiSection title="Response">
Returns the audio stream directly with appropriate `Content-Type` header (e.g., `audio/opus`, `audio/webm`).
  </ApiSection>
</ApiCard>
